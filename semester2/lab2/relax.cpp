...
mth::vec mth::lieqsys::Relax( matr const &A, vec const &b, vec const &x0, double Omega, double Epsilon, int &Steps ) {
  ...
  for (unsigned int y = 0; y < A.getH(); y++)
    for (unsigned int x = 0; x < A.getH(); x++) {
      if (x >= y)
        L[y][x] = 0;
      if (x <= y)
        R[y][x] = 0;
